...
tidy$Chromosome<-as.factor(tidy$Chromosome)
tidy$gene<-as.factor(tidy$gene)
str(tidy)
tidy <- select(tidy,-Chromosome)
data_summary(data = tidy,varname = "sce_per_chr",
             groupnames = c("gene","Length"))

tidy%>% group_by(c("gene","Length"))%>% summarize(sd(sce_per_chr))
df2=ddply(tidy,~c(Length),summarise,sd=sd(sce_per_chr),mean=mean(sce_per_chr))
df2$`c(Length)`<-as.numeric(df2$`c(Length)`)

ggplot(df2, aes(x='c(Length)', y=mean)) + 
  geom_line() +
  geom_point()+
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                position=position_dodge(0.05))

ggplot(tidy) + geom_errorbar(aes(x = Length,ymin=min(sce_per_chr),ymax=max(sce_per_chr),group=gene,color=gene), na.rm=TRUE)+
  geom_smooth(aes(Length,sce_per_chr,group=gene,color=gene),se=F,method="lm", na.rm=TRUE,size=2)+
  theme_classic(base_size = 25) +
  ylab("SCEs/CHR/LIB")+
  xlab("CHR LENGTH")+
