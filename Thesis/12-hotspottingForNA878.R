...
breaks.all.files = collectBreaksAllFiles(paste0("../NA878/data/",quality_good,".RData"))
hotspots = breakpointHotspotter(breaks.all.files)

str(hotspots)
hotspots$count<- as.factor(hotspots$count)
level=levels(hotspots$count)[1]
summaryOfHotspots = data.frame()

for (level in levels(hotspots$count)){
  
  tmp <- filter(hotspots, count==level)
  row = data.frame(chr=tmp[1,1],start = min(tmp$start),end=max(tmp$end),calls = nrow(tmp), libraries = length(levels(as.factor(tmp$ID))), width = max(tmp$width),level=level)
  
  summaryOfHotspots = rbind(row, summaryOfHotspots)  
}
...
savingAndPrinting <- function(hotspots,hotpath="HOTSPOT_EVENTS",printing=F,export=F,normalize=F,cfs=F,genomeInstability=T){
  ...
  numOfLibs = length(list.files("../NA878/data/"))
...
breaks.all.files <- collectBreaksAllFiles(datapath="../StraVa/DATA/rdata/")
