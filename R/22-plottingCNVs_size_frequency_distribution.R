...
dups=read.table("CNVs/FUCCI/30Mb_curatedCallsetDuplications.txt",header=T) %>% select(c(seqnames,start,end,width,gene))
dels=read.table("CNVs/FUCCI/30Mb_curatedCallsetDeletions.txt",header = T) %>% select(c(seqnames,start,end,width,gene))
...
p=data.frame(ID=levels(read.table("INPUT/05.breakpoints.metrics.quality.gene.txt",header=T)$library))
p$library <- p$ID
...
p = as.data.frame(p %>% group_by(gene)%>% summarize(n()))

m=merge %>% group_by(gene,type)%>% summarize(n())
q=merge(m,p,by="gene")
