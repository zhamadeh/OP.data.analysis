...
SNP1=read.table("CNVs/FUCCI/30Mb_curatedCallsetDuplications.txt",header=T) %>% select(c(seqnames,start,end,gene))
SNP2=read.table("CNVs/FUCCI/30Mb_curatedCallsetDeletions.txt",header = T) %>% select(c(seqnames,start,end,gene))
...
ggplot() +
  ...
  ggsave("OUTPUT/Karyograms/Del_Dup_overlay_ByGene_karyogram.png")


WT = data.frame()

filter(as.data.frame(get(load("CNVs/FUCCI/Data-AneuFinder/WT/WT_composite.bam_binsize_1e+05_stepsize_1e+05_reads.per.bin_659.87_.RData"))$breakpoints),seqnames=="chr15")
