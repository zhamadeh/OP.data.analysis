...
SNP = read.table("SCEs/NA878/na878_sces.bed")
...
ggplot() +
  ...
  geom_segment(data = SNP,
               aes(x = as.integer(chr) - 0.25, xend = as.integer(chr) + 0.25,
                   y = pos, yend = pos)
               ,color="#1d705d",size = 0.25) +
  theme_classic() +
  theme(text = element_text(size=18),axis.line=element_blank())+
  labs(x="",y="CHROMOSOME POSITION (Mb)")+
  ggsave("Output/Karyograms/karyogram.png")
