...
savingAndPrinting <- function(hotspots,hotpath="HOTSPOT_EVENTS",readspath="../karsan_aml/browser/"){
  ...
  for (level in levels(hotspots$count)){
    ...
    files2plot=paste0("../karsan_aml/rdata/",levels(tmp$ID),".RData")
...
breaks.all.files <- collectBreaksAllFiles(list.files("../karsan_aml/",full.names = T))
