...
breakpointSummary <- read.table("INPUT/2021/06.sisterChromatidExchanges.txt",header=T) %>% select(c("seqnames", "start" ,"end"  , "width" , "strand" , "genoT" ,"deltaW"  , "coverage" , "spikiness"    
                                                                                                   , "evenness.mean" ,"evenness.med" , "background" , "quality" , "gene","Library"))
breakpointSummary2 <- read.table("INPUT/06.sisterChromatidExchanges.txt",header=T) %>%  select(c("seqnames", "start" ,"end"  , "width" , "strand" , "genoT" ,"deltaW"  , "coverage" , "spikiness"    
                                                                                                 , "evenness.mean" ,"evenness.med" , "background" , "quality" , "gene","library"))
colnames(breakpointSummary2)=c("seqnames", "start" ,"end"  , "width" , "strand" , "genoT" ,"deltaW"  , "coverage" , "spikiness"    
                               , "evenness.mean" ,"evenness.med" , "background" , "quality" , "gene","Library")
breakpointSummary=rbind(breakpointSummary,breakpointSummary2)
...
suppressMessages(one <- as.data.frame(breakpointSummary %>% filter(gene=="RECQL1")  %>% group_by(seqnames) %>% dplyr::summarize(RECQL1=n())))
...
suppressMessages(rl <- as.data.frame(breakpointSummary %>% filter(gene=="RTEL1")  %>% group_by(seqnames)%>% dplyr::summarize(RTEL1=n())))
...
byChr <- merge(byChr,one,by="seqnames",all=T)
byChr <- merge(byChr,rl,by="seqnames",all=T)
byChr <- merge(byChr,all,by="seqnames",all=T)
byChr[is.na(byChr)] <- 0
write.table(byChr,"OUTPUT/SummaryTables/2019+2021/perChrom.txt",quote=F,row.names = F,col.names = T,sep="\t")
...
one <- (as.data.frame(breakpointSummary %>% filter(gene=="RECQL1")  %>% droplevels()))# group_by(seqnames)%>% summarize(n()))
numOfLibsPerGene= add_row(numOfLibsPerGene, gene="RECQL1",n=as.numeric(length(levels(one$Library))))
...
rl <- (as.data.frame(breakpointSummary %>% filter(gene=="RTEL1")  %>% droplevels()))# group_by(seqnames)%>% summarize(n()))
numOfLibsPerGene= add_row(numOfLibsPerGene, gene="RTEL1",n=as.numeric(length(levels(rl$Library))))
numOfLibsPerGene= add_row(numOfLibsPerGene, gene="ALL",n=as.numeric(length(levels(breakpointSummary$Library))))
write.table(numOfLibsPerGene,"OUTPUT/SummaryTables/2019+2021//numOfLibsPerGene.txt",quote=F,row.names = F,col.names = F,sep="\t")


numOfLibsPerGene=read.table("OUTPUT/SummaryTables/2019+2021//numOfLibsPerGene.txt",header=F)
...
byChr<- read.table("OUTPUT/SummaryTables/2019+2021//perChrom.txt",header=T)

for (i in c("RECQL1","WT","BLM.RECQL5","RECQL5","RTEL1","BLM","ALL")){
  #print(byChr[,i])
...
ggplot(tidy) +  geom_smooth(aes(Length,sce_per_chr,group=gene,color=gene),se=F,method="lm", na.rm=TRUE,size=2)+
  ...
  ggsave("OUTPUT/Plots/2019+21_chrLength.png")
...
for (row in 1:nrow(test)){
  for (letter in c("a","b","c","d","e","f")){
    ...
    else if (test[row,letter]=="WT" | test[row,letter]=="wt"){
      test[row,"gene"]="WT"
    }
...
res.aov=aov(sces~gene,data=test)
tuk = as.data.frame(TukeyHSD(res.aov)$gene)
tuk$pair = row.names(tuk)
tuk=tuk %>% separate(pair,c("a","b"),"[-]")
tuk=subset(tuk,a == "WT") %>% select(c(`p adj`,b))
tuk$`p adj`=formatC(signif(tuk$`p adj`,digits=3), digits=3,format="fg", flag="#")
...
ggplot(test) + geom_jitter(aes(gene,sces), color="#4a4a4a")+ geom_boxplot(aes(gene,sces),width=0.1,coef = 5,fill="#828bb5") +
  ...
  geom_text(data=tuk,aes(x=b,y=54,label=paste0("p = ",`p adj`)))+
  geom_text(data=tuk,aes(x=b,y=50,label=sig))+
  labs(y="SCEs/cell")#+
  ggsave("OUTPUT/Plots/SCEperGene2021+2019.png")



my_comparisons <- list( c("RTEL1","WT"),c("RECQL5","WT"), c("BLM/RECQL5","WT"), c("RECQL1","WT"),c("BLM","WT") )
...
suppressMessages(suppressWarnings(ggplot(breakpointSummary) + geom_smooth(aes(Reads_per_Mb, width,color=gene),se=F) +
                                    ...
                                    ggsave("OUTPUT//Plots/resolutionVsDepth2021+2019.png")))
 

sce_summary=data.frame(gene=character(),SCE=numeric(),mean_resolution=numeric(),median_resolution=numeric())
one<- filter(breakpointSummary, gene=="RECQL1")
sce_summary<- add_row(sce_summary,gene="RECQL1",SCE=nrow(one),mean_resolution=mean(one$width),median_resolution=median(one$width))
rl<- filter(breakpointSummary, gene=="RTEL1")
sce_summary<- add_row(sce_summary,gene="RTEL1",SCE=nrow(rl),mean_resolution=mean(rl$width),median_resolution=median(rl$width))
...
write.table(sce_summary,"OUTPUT/SummaryTables/2019+2021//SCE_summary.txt",quote=F,row.names = F,col.names = T,sep="\t")
...
suppressMessages(suppressWarnings(ggplot(breakpointSummary) + stat_ecdf(aes(width,color=gene)) +
                                    ...
                                    ggsave("OUTPUT//Plots/breakpointResolution2021+2019.png")
